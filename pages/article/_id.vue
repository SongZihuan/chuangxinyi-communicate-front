            ...
            <div class="article-header">
              <div class="article-item-left">
                <a
                  ...
                  target="_blank"
                >
                  <img v-lazy="$config.AVATAR_URL + '?uid=' + article.user.uid" class="avatar" />
                </a>
              </div>
...
export default {
  ...
  async asyncData({ $axios, params, error }) {
    ...
    const [
      ...
      $axios.get('/api/articles/user/newest/' + article.user.id),
      $axios.get('/api/articles/related/' + article.articleId)
    ])
  ...
  computed: {
    isOwner() {
      return (
        ...
        this.article &&
        this.$store.state.auth.currentUser.id === this.article.user.id
      )
    }
  },
  methods: {
    ...
    async addFavorite(articleId) {
      try {
        if (this.favorited) {
          ...
          await this.$axios.post('/api/article/' + articleId + '/favorite')
          this.favorited = true
